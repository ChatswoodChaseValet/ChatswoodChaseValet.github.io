...
3. **Claim or assign it.** Anyone can tap **I'll get it** to take a job themselves. On **desktop**, the person at the counter can instead use the **Assign to…** dropdown to hand the job to a specific bag-enabled staff member — it then **rings on that person's phone** (sound + vibration + an "Assigned to you" popup, while the app is open) and shows as **YOUR JOB** (highlighted) in their Bags queue. Picked the wrong person? Open the job and use the **Reassign…** dropdown to switch them, or choose **↺ Unassign** to put it back in the open queue.
